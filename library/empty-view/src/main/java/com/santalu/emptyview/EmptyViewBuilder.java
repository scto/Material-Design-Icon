...
public class EmptyViewBuilder {
    ...
    EmptyViewBuilder(EmptyView emptyView, @NonNull AttributeSet attributeSet) {
        ...
        try {
			...
			int textColorPrimary = defaultTextColor;
			int textColorSecondary = defaultTextColor;
			{
				TypedArray aa = context.obtainStyledAttributes(new int[]{android.R.attr.textColorPrimary,android.R.attr.textColorSecondary});
				if (aa.hasValue(0)) {
					textColorPrimary = aa.getColor(0, textColorPrimary);
				}
				if (aa.hasValue(1)) {
					textColorSecondary = aa.getColor(1, textColorSecondary);
					defaultTextColor = textColorSecondary;
				}
				aa.recycle();
			}
	...
	public EmptyViewBuilder setEmptyDrawableTint(int emptyDrawableTint) {
